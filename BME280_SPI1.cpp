...
void writeReg1(uint8_t reg_address, uint8_t data) {
    PORTC &= ~(1<<PC1);
...
void readReg1(uint8_t reg_address, int numBytes, uint8_t * data) {
...
void readTrim1() {
	...
	readReg1(0x88, 24, data);
	readReg1(0xA1,  1, &data[24]);
	readReg1(0xE1,  7, &data[25]);
...
double readTemperature1(void) {
    signed long int temp_raw;
    signed long int var1, var2;
  
    uint8_t data[8];

    readReg1(0xFA, 3, data);
...
double readHumidity1(void) {
    unsigned long int hum_raw;
    signed long int v_x1;
  
    uint8_t data[8];

    readReg1(0xFD, 3, data);
